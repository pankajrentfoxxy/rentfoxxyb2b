...
export default function AsAsListClient({ initialData }: { initialData: AsAsRow[] }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return initialData.filter(
      (row) =>
        ...
        row.unitsDisplay.toLowerCase().includes(q),
    );
  }, [initialData, searchQuery]);

  const columns: Column<AsAsRow>[] = [
    {
      header: "Title",
      key: "title",
      width: 220,
      ...
      render: (row) => (
        <Link href={`/admin/asas/${row.id}`}>
          <button
            ...
            aria-label={`View listing ${row.title}`}
          >
            <Eye size={16} />
          </button>
        </Link>
  ...
  return (
    <CommonTable
      ...
      emptyMessage="No AsAs listings yet."
    />
  );
}
