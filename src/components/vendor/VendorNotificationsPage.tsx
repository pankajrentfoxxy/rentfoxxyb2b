...
export function VendorNotificationsPage() {
  ...
  const load = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/vendor/notifications?page=${p}&limit=${limit}`);
  ...
  async function markAllRead() {
    await fetch("/api/vendor/notifications/read-all", { method: "PUT" });
    load(page);
  }

  async function markOne(id: string) {
    await fetch(`/api/vendor/notifications/${id}`, { method: "PATCH" });
    load(page);
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Bids, orders, and platform updates for your store.</p>
        </div>
        <button
          type="button"
          onClick={markAllRead}
        ...
        <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
          {items.length === 0 ? (
            ...
            items.map((n) => (
              <li
                key={n.id}
                className={cn("flex gap-3 px-4 py-4", !n.isRead && "bg-accent-light/30")}
              >
