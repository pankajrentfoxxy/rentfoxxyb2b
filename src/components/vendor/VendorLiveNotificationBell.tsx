...
export function VendorLiveNotificationBell() {
  const [items, setItems] = useState<Notice[] | undefined>(undefined);

  const load = useCallback(async () => {
    const res = await fetch("/api/vendor/notifications?limit=8");
    if (!res.ok) return;
  ...
  return (
    <NotificationBell
      notifications={items}
      viewAllHref="/vendor/notifications"
      readAllPath="/api/vendor/notifications/read-all"
      onMarkAllRead={() => load()}
    />
  );
}
